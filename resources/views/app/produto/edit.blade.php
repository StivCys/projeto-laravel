            ...
            <form method="post" action='{{ route('produto.update',['produto'=>$produto->id])}}'>
                ...
                <select name='unidade_peso_id'>
                    ...
                    @foreach($unidades_peso as $key=> $unidadePeso)
                        <option value={{$unidadePeso->id}} {{$unidadePeso->id==$produto->unidade_peso_id? 'selected':'' }}>{{ $unidadePeso->unidade}} - {{ $unidadePeso->descricao }}</option>
                    @endforeach
                </select>
                ...
                <select name='unidade_id' >
                    ...
                    @foreach($unidades as $key=> $unidade)
                        <option value={{$unidade->id}} {{$unidade->id==$produto->unidade_id? 'selected':'' }}>{{ $unidade->unidade}} - {{ $unidade->descricao }}</option>
                    @endforeach
                </select>
